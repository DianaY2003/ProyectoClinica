<template>
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" :value="pacientes" v-model ="search" datakey="id"
                :paginator="true" :rows="10" :filters="filters"
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown" :rowsPerPageOptions="[5,10,25]"
                currentPageReportTemplate="Mostrar {first} de {last} of {totalRecords} pacientes" size="small" tableStyle="min-with: 50rem">
                ...
                <template #body="slotProps">
                    ...
                    <Button icon="pi pi-file" outlined rounded class="mr-2" @click="verExpediente(slotProps.data)"/>
        ...
        <Dialog v-model:visible="pacienteDialog" :style="{width: '550px'}" :header="formTitle" :modal="true" class="p-fluid">
            <div class="formgrid grid row">
            <div class="field col">
                <label for="nombre">Nombre</label>
                <InputText id="nombre" v-model="paciente.nombre" required="true" autofocus :class="{'p-invalid': submitted && !paciente.nombre}" />
                <small class="p-error" v-if="submitted && !paciente.nombre">Nombre es requerido.</small>
            </div>
               <div class="field col">
                <label for="apellido">Apellido</label>
                <InputText id="apellido" v-model="paciente.apellido" required="true" :class="{'p-invalid': submitted && !paciente.apellido}" />
                <small class="p-error" v-if="submitted && !paciente.apellido">Apellido es requerido.</small>
            </div>
            </div>
               ...
               <div class="formgrid grid row">
                ...
                <InputText id="dui" v-model="paciente.dui" :class="{'p-invalid': submitted && !paciente.dui}" required maxlength="9" minlength="9" autofocus />
                <small class="p-error" v-if="submitted && !paciente.dui">DUI es requerido.</small>
                 </div>
                <div class="field col">
                <label for="telefono">Telefono</label>
                <!-- <InputNumber id="telefono" v-model.trim="paciente.telefono"  integeronly :class="{'p-invalid': submitted && !paciente.telefono}"/> -->
                <InputText id="telefono" v-model="paciente.telefono" :class="{'p-invalid': submitted && !paciente.telefono}" required maxlength="8" minlength="8" autofocus />
                <small class="p-error" v-if="submitted && !paciente.telefono">Telefono es requerido.</small>
              </div>
            </div>
            <div class="formgrid grid row">
                <div class="field col">
                <label for="fecha_nacimiento">Fecha Nacimiento</label>
                <Calendar id="fecha_nacimiento" v-model="paciente.fecha_nacimiento" :readonly-input="true" showIcon />
               </div>
                <div class="field col">
                <label for="sexo">Genero</label>
                <Dropdown v-model="paciente.sexo" :options="opcionesGenero" optionLabel="label" optionValue="value" placeholder="Seleccionar Genero" />
               </div>
            </div>
               <div class="formgrif grid row">
                <div class="field col">
                <label for="estado_civil">Estado Civil</label>
                <!-- <InputText id="estado_civil" v-model.trim="paciente.estado_civil" /> -->
                <Dropdown v-model="paciente.estado_civil" :options="opcionesEstadoCivil" optionLabel="label" optionValue="value" placeholder="Seleccionar estado civil" />
               </div>
                <div class="field col">
                      <label for="categoria">Distrito</label>
                    <Dropdown v-model="paciente.distrito_id" :options="distritos" optionLabel="nombre" optionValue="id"  placeholder="Seleccione distrito" class="w-full md:w-14rem"> </Dropdown>
                    <small class="p-error" v-if="submitted && !paciente.distrito_id">Seleccione un distrito.</small>
                </div>
               </div>
       ...
       data(){
        ...
        methods:{
            ...
            async fetchDistritos(){
                await this.axios.get(`/api/distritos`)
                .then(response => {
        ...
        async saveOrUpdate(){
            ...
            if(me.paciente.nombre.trim()){
                ...
                if (accion == "add"){
                    ...
                    await this.axios.post(`/api/pacientes`,this.createFormData(), headers)
                    .then(response => {
                    ...
                    await this.axios.put(`/api/pacientes/${me.paciente.id}`,this.paciente)
                    .then(response => {
            ...
            switch(accion){
                case "add":
                    ...
                    case "upd":
                     ...
                     case "del":
                        if (statusCode == 205){
                            me.pacientes.splice(me.editedPaciente,1);
                            Toast.fire({
                                icon: 'success',
                                'title': 'pacientes Eliminado...!'
                            });
                        }else{
                            this.$swal.fire({
                                icon: 'error',
                                text: 'No se puede eliminar , ya  existen reservas registradas en este paciente'
                            });
                        } 
                       break;
                }  
